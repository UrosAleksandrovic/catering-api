...
namespace Catering.Application.Handlers
{
    internal class GetActiveOrdersOfItemsHandler : IRequestHandler<GetActiveOrdersOfItem, List<Order>>
    {
        ...
        Task<List<Order>> IRequestHandler<GetActiveOrdersOfItem, List<Order>>.Handle(GetActiveOrdersOfItem request, CancellationToken cancellationToken)
        {
            return _orderRepository.GetActiveOrdersByItemAsync(request.ItemId);
        }
    }
}
